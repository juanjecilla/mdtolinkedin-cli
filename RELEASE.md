# Release Preparation Guide

## Pre-Release Checklist

### 1. Verify Code Quality

```bash
# Set up Rust toolchain (if not already done)
rustup default stable

# Check code compiles
cargo check

# Run all tests
cargo test

# Check formatting
cargo fmt --check

# Run linter
cargo clippy -- -D warnings

# Format code (if needed)
cargo fmt
```

### 2. Crates.io Publish (per release)

```bash
# Dry run to validate packaging metadata
cargo publish --dry-run

# Publish (requires crates.io token)
cargo publish
```

### 3. Files to Commit

The following files should be committed:

**Source Code:**
- `src/cli.rs`
- `src/converter.rs`
- `src/io.rs`
- `src/main.rs`
- `src/unicode.rs`

**Tests:**
- `tests/integration.rs`

**CI/CD:**
- `.github/workflows/ci.yml`
- `.github/workflows/release.yml`

**Documentation:**
- `README.md` (updated with installation instructions)
- `AGENTS.md` (project guidance)

**Configuration:**
- `Cargo.toml`
- `Cargo.lock`
- `.gitignore`
- `.rpm/` (cargo-rpm spec file)
- `LICENSE`

**Optional (can be excluded):**
- `test_input.md`
- `test_output.txt`
- `long_input.md`
- `docs/` (documentation for development)

### 4. Git Commands for Release

```bash
# Stage all changes
git add src/ tests/ .github/ README.md AGENTS.md Cargo.toml Cargo.lock .gitignore IMPROVEMENTS.md

# Commit changes
git commit -m "feat: initial release v0.1.0

- Implement Unicode Bold/Italic conversion
- Add Markdown to LinkedIn converter
- Add CLI with file/stdin I/O support
- Add character limit warning
- Add integration tests
- Add CI/CD workflows
- Update documentation"

# Create version tag
git tag -a v0.1.0 -m "Release v0.1.0 - Initial release"

# Push commits and tags
git push origin main
git push origin v0.1.0
```

### 5. After Release

Once GitHub Actions builds the release:

1. **Verify release assets**
   ```bash
   ./scripts/verify_release_assets.sh v0.1.0
   ```

2. **Homebrew formula updates**
   - Automated via `.github/workflows/homebrew.yml` if `HOMEBREW_TAP_TOKEN` is configured
   - Manual update:
     ```bash
     gh release download v0.1.0 --pattern "SHA256SUMS.txt" --dir dist
     python3 scripts/update_homebrew_formula.py \
       --version v0.1.0 \
       --checksums dist/SHA256SUMS.txt \
       --tap-dir /path/to/homebrew-tap
     ```

3. **Scoop manifest updates**
   ```bash
   gh release download v0.1.0 --pattern "SHA256SUMS.txt" --dir dist
   python3 scripts/update_scoop_manifest.py --version v0.1.0 --checksums dist/SHA256SUMS.txt
   ```

4. **Release checksums**
   - `SHA256SUMS.txt` is generated by the release workflow and attached to the GitHub Release
   - Optional GPG signature is attached when `GPG_PRIVATE_KEY` and `GPG_PASSPHRASE` secrets are set
   - `.deb` and `.rpm` Linux packages are also attached by the release workflow

### 6. Homebrew Formula Template

```ruby
class Mdtolinkedin < Formula
  desc "Convert Markdown to LinkedIn-compatible text"
  homepage "https://github.com/juanjecilla/mdtolinkedin-cli"
  version "0.1.0"

  on_macos do
    on_arm do
      url "https://github.com/juanjecilla/mdtolinkedin-cli/releases/download/v0.1.0/mdtolinkedin-macos-aarch64.tar.gz"
      sha256 "REPLACE_WITH_SHA256"
    end
    on_intel do
      url "https://github.com/juanjecilla/mdtolinkedin-cli/releases/download/v0.1.0/mdtolinkedin-macos-x86_64.tar.gz"
      sha256 "REPLACE_WITH_SHA256"
    end
  end

  on_linux do
    url "https://github.com/juanjecilla/mdtolinkedin-cli/releases/download/v0.1.0/mdtolinkedin-linux-x86_64.tar.gz"
    sha256 "REPLACE_WITH_SHA256"
  end

  def install
    bin.install "mdtolinkedin"
  end

  test do
    assert_match "mdtolinkedin", shell_output("#{bin}/mdtolinkedin --version")
  end
end
```

## Current Status

✅ All source code implemented  
✅ All tests fixed and verified  
✅ CI/CD workflows created  
✅ Documentation updated  
✅ Code structure verified  
⏳ Ready for release (pending Rust toolchain setup for final verification)
